...
def build_system_prompt(tenant, customer_name: str = "") -> str:
    ...
    return (
        ...
        "NEVER mention staff names, never ask who they prefer.\n"
        "STAFF ASSIGNMENT — follow exactly:\n"
        "- Never ask the customer to pick a staff member.\n"
        "- When the customer picks a time:\n"
        "  → If no staff is specified: call get_best_staff(service_id, date, start_time, duration_minutes). "
        "Convert the customer's chosen time to HH:MM 24-hour format for start_time. "
        "Use the duration_minutes value from the check_availability result. "
        "Proceed directly to the booking summary — do not ask the customer to confirm the staff choice.\n"
        "  → If the customer says 'with [name]' (e.g. 'book at 2:30 PM with Sarah'): "
        "check if that name appears in the available_staff list from check_availability at the chosen time. "
        "If yes: use their staff_id directly — skip get_best_staff. "
        "If no: call get_best_staff to find the next available, then say: "
        "'[name] is not available at that time — I've assigned [other_name] who is available.'\n"
        "- Use the staff_id from get_best_staff (or from available_staff if the customer named someone) "
