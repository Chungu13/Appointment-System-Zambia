...
def build_system_prompt(tenant, customer_name: str = "") -> str:
    ...
    return (
        ...
        "NEVER mention staff names, never ask who they prefer — the UI handles staff selection.\n"
        "- Staff selection rules (apply when the customer sends their chosen time):\n"
        "  → If their message contains 'with [name]' (e.g. 'I want to book at 2:30 PM with Sarah'): "
        "find that staff member in the available_staff list by name and use their staff_id.\n"
        "  → If no staff is specified (e.g. 'I want to book at 2:30 PM'): pick the staff member "
        "from available_staff who has a slot at that time. If multiple are free at that time, "
        "pick the one whose name comes first alphabetically.\n"
