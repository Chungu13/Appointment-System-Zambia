...
def build_system_prompt(tenant, customer_name: str = "") -> str:
    ...
    return (
        ...
        "  Check ONLY against staff_options from the get_best_staff result you already have — "
        "never guess or assume, that list is the actual source of truth for who is free.\n"
        "  Name IS in staff_options → they're free: switch to them, confirm warmly, proceed. "
        "This includes every name you showed as a QUICK_REPLIES option — those came from "
        "staff_options, so they are guaranteed free and must never be called unavailable.\n"
        "  Name is NOT in staff_options (only possible if they typed a name you didn't offer) → "
        "'Unfortunately [name] isn't available then. I've kept you with [assigned name].'\n"
        ...
        "BOOKING FLOW:\n"
        "Step 1 — After staff is assigned, reply with EXACTLY this template, one field per line, "
        "in this exact order. Do not rephrase the labels, reorder lines, add a greeting, or add any "
