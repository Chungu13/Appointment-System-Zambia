...
class BookingsMutation:
    ...
    def add_to_waitlist(
        ...
        if preferred_staff_id:
            preferred_staff = User.objects.filter(pk=preferred_staff_id, is_active=True).first()
        ...
        entry = (
            Waitlist.objects
            .select_related("customer", "service", "preferred_staff")
            .get(pk=entry.pk)
        )
        return waitlist_to_type(entry)
